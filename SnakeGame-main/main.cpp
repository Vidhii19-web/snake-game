...
public:
    ...
    void GameOver()
    {
         lastScore = score; // Store score before reset
        snake.Reset();
        food.position = food.GenerateRandomPos(snake.body);
        running = false;
        score = 0;
...
if (!game.running) {
            ...
            DrawRectangleLinesEx(Rectangle{(float)offset - 5, (float)offset - 5, (float)cellSize * cellCount + 10, (float)cellSize * cellCount + 10}, 5, darkGreen);
            DrawText("Snake game", offset - 5, 20, 40, darkGreen);
            DrawText("Game Over", offset + 100, offset + cellSize * cellCount / 2, 50, RED);
            DrawText(TextFormat("Final Score: %i", game.lastScore), offset + 80, offset + cellSize * cellCount / 2 + 60, 30, darkGreen); // Use lastScore
            DrawText("Press R to continue", offset + 80, offset + cellSize * cellCount / 2 + 100, 30, DARKGRAY);
            // Do NOT call game.Draw() here, so snake and apple are hidden
